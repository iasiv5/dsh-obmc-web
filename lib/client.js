...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Shared status store: the whole panel polls /bmc-status ONCE (not one
    // loop per component), and every subscriber renders the same snapshot.
    // ?probe=1 makes the host verify the tunnel live; the returned `ok`
    // field is the authoritative tunnel verdict for the chip. Cadence is
    // 15s, 2s while a discovery round is running. Tunnel-affecting actions
    // (save/probe/relay install) call refreshStatus() so the chip follows
    // the action's own verdict within a second or two instead of lagging
    // behind it by up to a full interval.
    const statusStore = { data: null, listeners: new Set(), timer: null, inFlight: false, pending: false }
    ...
    function scheduleNext() {
      clearTimeout(statusStore.timer)
      if (statusStore.listeners.size === 0) return
      const discovering = statusStore.data !== null && statusStore.data.discovering
      statusStore.timer = setTimeout(pollStatus, discovering ? 2_000 : 15_000)
    }

    function applyStatus(next) {
      statusStore.data = next
      for (const listener of statusStore.listeners) listener(next)
    }

    async function pollStatus() {
      if (statusStore.inFlight) {
        statusStore.pending = true
        return
      }
      statusStore.inFlight = true
      try {
        applyStatus(await fetchStatus())
      } finally {
        statusStore.inFlight = false
      }
      // A refresh requested mid-flight re-runs once the stale round ends,
      // so an action's immediate probe is never swallowed by an older one.
      if (statusStore.pending && statusStore.listeners.size > 0) {
        statusStore.pending = false
        pollStatus()
        return
      }
      scheduleNext()
    ...
    /** Immediate re-probe: cancels the pending interval tick. */
    ...
    function useBmcStatus() {
      const [status, setStatus] = React.useState(() => statusStore.data)
      React.useEffect(() => {
        const listener = next => setStatus(next)
        statusStore.listeners.add(listener)
        if (statusStore.listeners.size === 1) pollStatus()
        return () => {
          statusStore.listeners.delete(listener)
          if (statusStore.listeners.size === 0) clearTimeout(statusStore.timer)
        }
      }, [])
      return status
    }
    ...
    function DeployConfigForm() {
      ...
      // One shared grid: every field gets the same track width, so all five
      // inputs render the same length regardless of their label. auto-fit
      // keeps two columns on the settings panel and collapses to one on
      // narrow windows.
      const field = (label, key, placeholder) => React.createElement(
        'label',
      ...
      // saved values), then start an SSH discovery round and wait for THAT
      // round to end (roundEndedAt after our start), whatever it concludes.
      const probe = async () => {
        setBusy(true)
        setMsg(null)
        try {
          ...
          const kick = await fetch(STATUS_URL, { method: 'POST', cache: 'no-store' })
            .then(r => r.json()).catch(() => null)
          if (kick && kick.started === false && kick.reason && !/already running/.test(kick.reason)) {
            setMsg({ ok: false, text: kick.reason })
            return
          }
          ...
          for (let i = 0; i < 45; i++) {
            ...
            if (status && (status.roundEndedAt || 0) > startAt) break
          }
      ...
      const button = (label, onClick, disabled, primary, title, runningLabel) => React.createElement(
        'button',
        {
          type: 'button',
          onClick,
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          React.createElement('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '10px' } },
            field('SSH 跳板机 user@host（探测必填）', 'sshTarget', 'user@你的跳板机地址'),
            field('跳板机 SSH 端口', 'sshPort', '默认 22'),
            field('BMC 地址（留空可自动探测）', 'bmcTarget', 'ip 或 ip:port，如 192.0.2.10'),
            field('BMC 所在网段（可选）', 'subnet', 'a.b.c，默认自动判断'),
            field('relay 本地端口', 'relayPort', '默认 18443'),
          ),
          React.createElement(
            'div',
            { style: { display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' } },
            button('保存配置', save, false, true, undefined, '保存中…'),
            button(probeLabel, probe, probeDisabled, false, probeTitle, '探测中…'),
            button(relay && relay.installed ? '重装/更新 relay 服务' : '安装并启动 relay 服务', installRelay, false, false, undefined, '安装中…'),
