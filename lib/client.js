    ...
    function OpenButton() {
      ...
      return React.createElement(
        'button',
        {
          ...
          title: unconfigured ? '尚未配置 BMC 地址：先在下方部署配置里填写并保存' : undefined,
          style: {
    ...
    function DeployConfigForm() {
      ...
      const save = async () => {
        ...
        try {
          ...
          if (!response.ok) {
            setMsg({ ok: false, text: (data.errors || [data.error || '保存失败']).join('；') })
          } else {
            setMsg({
      ...
      const installRelay = async () => {
        ...
        try {
          ...
          if (!response.ok) {
            ...
            setMsg({
              ok: data.verified !== false,
              text: data.verified === true ? 'relay 服务已安装并启动，隧道验证通过' : 'relay 服务已写入，但隧道验证未通过（查看 systemctl --user status obmc-web-relay）',
            })
            await load()
          }
        } catch (error) {
      ...
      const button = (label, onClick, disabled, primary) => React.createElement(
        'button',
        {
          type: 'button',
          onClick,
      ...
      return React.createElement(
        ...
        unconfigured
          ? React.createElement(
            ...
            '尚未配置 BMC 地址：展开下方「部署配置」，填写 BMC 地址与 SSH 跳板机并保存，然后安装 relay 服务即可打通隧道。',
          )
          : null,
