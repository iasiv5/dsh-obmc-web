...
export function apply(ctx) {
  ...
  register({
    ...
    handler: guarded((req, res) => {
      ...
      systemdManagerCall(['Reload'])
        .then(() => relayRestart())
        .then(verified => relayInfo().then(relay => writeJson(res, 200, { installed: true, relay, verified })))
        .catch(error => writeJson(res, 500, { errors: [`systemd 操作失败：${error.message}`] }))
    }),
