 ...
 *
 * Embed-hostile and origin-hostile response headers are removed
 * (X-Frame-Options, HSTS, CSP frame-ancestors, WWW-Authenticate so bmcweb's
 * 401 Basic challenges can never pop the browser's native credential
 * dialog — the BMC SPA logs in via POST /login JSON + cookie — and
 * Clear-Site-Data, whose origin-wide wipe on BMC logout would otherwise
 * destroy the GUI's session cookie). Non-/login 401s are degraded
 * to 403: the BMC SPA redirects the whole tab to root-absolute /login on
 * any other 401, which on this origin lands in the GUI's auth gate and
 * bounces the browser into the DSH app. Everything else passes through
 * untouched; no credentials are stored anywhere.
 *
...
// clear-site-data is dropped because bmcweb's logout answers
// Clear-Site-Data: "cache","cookies","storage" — an instruction to wipe
// EVERY cookie/cache/storage entry for the ORIGIN. Through this same-origin
// proxy that origin is the GUI's: one BMC logoff would otherwise destroy
// the DSH session cookie (and DSH's localStorage) too. BMC-side logout
// still works without it: bmcweb expires its SESSION cookie via an
// explicit Set-Cookie (passes through) and destroys the session
// server-side; only the origin-wide nuke is suppressed.
const DROP_RESPONSE = new Set([
  ...HOP, 'x-frame-options', 'strict-transport-security', 'www-authenticate',
  'clear-site-data',
])
...
const bmcState = {
  ...
  lastError: null,
  lastErrorAt: null,        // epoch ms — lets the UI tell fresh errors apart
  ...
  roundStartedAt: null,     // epoch ms — discovery round bookkeeping
  roundEndedAt: null,       // epoch ms — UI polls until this passes its start
  startedAt: Date.now(),
}
...
async function discoverBmc(manual) {
  ...
  bmcState.roundStartedAt = Date.now()
  bmcState.roundEndedAt = null
  ...
  ;(async () => {
    try {
      ...
      if (!result.found) {
        ...
        // Surface the REAL cause: an SSH-level failure (wrong port/host/key)
        // must not masquerade as "no bmcweb speaker found".
        const stderrTail = (result.stderr || '').split('\n').map(l => l.trim()).filter(Boolean).pop() || ''
        if (result.error) bmcState.lastError = result.error
        else if (result.code !== 0 || stderrTail) bmcState.lastError = `SSH 探测未成功（检查跳板机地址/端口/免密登录）：${stderrTail || `exit code ${result.code}`}`
        else bmcState.lastError = `跳板机上未发现 bmcweb（网段：${envConfig.subnet || '自动枚举直连网段'}）`
        bmcState.lastErrorAt = Date.now()
        return
      }
      ...
      if (switched) {
        ...
        try {
          ...
          // Bootstrap case: the relay unit is not installed yet — the
          // address was still found and persisted; tell the user the next
          // step instead of reporting a cryptic systemd error.
          if (/not loaded|not found/i.test(error.message || '')) switchNote = 'relay 服务未安装'
          else throw error
        }
      } else {
      ...
      // The env file may have gained BMC_TARGET/OBMC_SUBNET — refresh the
      // in-memory config so GET /bmc-config reflects it without a restart.
      ...
      if (!verified) {
        bmcState.lastError = switchNote
          ? `已发现 BMC ${result.found} 并写入配置；${switchNote}，点「安装并启动 relay 服务」即可打通隧道`
          : `找到 ${result.found} 但隧道切换未验证`
        bmcState.lastErrorAt = Date.now()
      }
    } catch (error) {
      bmcState.phase = 'failed'
      ...
      bmcState.roundEndedAt = Date.now()
      discovering = false
    }
  })()
  return { started: true }
...
function proxyRequest(req, res, targetPath, log) {
  ...
  const upstream = https.request(
    ...
    (up) => {
      ...
      // The BMC SPA's axios interceptor treats ANY 401 outside /login as
      // "session lost" and answers with `window.location = "/login"` — a
      // ROOT-ABSOLUTE navigation that escapes the /bmc prefix. On this
      // origin /login belongs to the GUI's auth gate, which 307s such
      // navigations into the DSH app, yanking the browser out of the BMC
      // console entirely (seen on the first console open after a DSH
      // restart: the SPA boots with stale localStorage login hints, probes
      // /redfish/... while logged out, and the 401 bounce follows). Degrade
      // every non-/login 401 to 403: the SPA's 403 path (setUnauthorized)
      // is an in-place toast, never a navigation. /login keeps its true
      // status so the login form's error handling and the CSRF probe keep
      // working.
      const status = up.statusCode === 401 && !/^\/login(\?|$)/.test(targetPath) ? 403 : up.statusCode
      res.writeHead(status, raw)
      up.pipe(res)
    },
  )
  ...
  register({
    ...
    handler: guarded((req, res) => {
      ...
      live.then(probe => writeJson(res, 200, {
        ...
        lastErrorAt: bmcState.lastErrorAt,
        lastUpstreamError: bmcState.lastUpstreamError,
        lastDiscovery: bmcState.lastDiscovery,
        roundStartedAt: bmcState.roundStartedAt,
        roundEndedAt: bmcState.roundEndedAt,
        discovering,
