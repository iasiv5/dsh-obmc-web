 ...
 *
 * All site-specific values (BMC address, SSH jump host, link subnet) live
 * in the env file — nothing about your network is hardcoded here.
 *
...
// Shipped code contains no site-specific defaults; see deploy/ for examples.
...
function readEnvConfig() {
  const config = { bmcTarget: null, sshTarget: null, sshPort: 22, subnet: null }
  try {
    ...
    for (const line of text.split('\n')) {
      ...
      else if (key === 'OBMC_SSH_PORT' && /^\d{1,5}$/.test(value) && Number(value) <= 65_535) config.sshPort = Number(value)
...
/** Update only the BMC_TARGET line; preserve every other deployment key. */
function writeTargetEnvFile(ip) {
  fs.mkdirSync(path.dirname(ENV_FILE), { recursive: true })
  let text = ''
  try { text = fs.readFileSync(ENV_FILE, 'utf8') } catch { /* first write */ }
  const line = `BMC_TARGET=${ip}:443`
  if (/^BMC_TARGET=.+$/m.test(text)) text = text.replace(/^BMC_TARGET=.+$/m, line)
...
function tunnelRequest(requestPath, timeoutMs = 8000) {
  return new Promise(resolve => {
    const request = https.request(
      { host: UP_HOST, port: UP_PORT, method: 'GET', path: requestPath, agent, timeout: timeoutMs },
      response => {
        response.resume()
...
function systemdManagerCall(args) {
  return new Promise((resolve, reject) => {
    ...
    execFile('busctl', ['--user', 'call', 'org.freedesktop.systemd1', '/org/freedesktop/systemd1',
      'org.freedesktop.systemd1.Manager', ...args], { env, timeout: 20_000 }, (error, stdout) => {
 ...
 * first (sub-second when the BMC is merely renumbered), full TCP-443 sweep
 * as fallback. Candidates are fingerprinted by their /redfish/v1 answer.
 * Fixed script body; the only interpolated value is the validated subnet
 * (three 0-255 octets, never free-form text).
 */
function buildDiscoveryScript(subnet) {
  return `# dsh-obmc-web BMC discovery on the jump host (BMC link network: ${subnet}.0/24)
SUBNET=${subnet}
cands=""
[ -r /var/lib/misc/dnsmasq.leases ] && cands="$cands $(awk '{print $3}' /var/lib/misc/dnsmasq.leases 2>/dev/null)"
cands="$cands $(ip neigh show dev eth0 2>/dev/null | awk '{print $1}' | grep -E "^$SUBNET\\.")"
cands=$(printf '%s\\n' $cands | sort -u)
...
async function switchTunnelTarget(ip) {
  ...
  await systemdManagerCall(['RestartUnit', 'ss', 'obmc-web-relay.service', 'replace'])
...
async function discoverBmc(manual) {
  if (envConfig.sshTarget === null || envConfig.subnet === null) {
    return { started: false, reason: 'discovery not configured: set OBMC_SSH_TARGET (and optionally OBMC_SUBNET) in the relay env file' }
  }
  ...
  ;(async () => {
    try {
      ...
      if (!result.found) {
        ...
        bmcState.lastError = result.error || `no bmcweb speaker found on the rpi (${previous})`
        return
      }
...
function proxyRequest(req, res, targetPath, log) {
  ...
  const upstream = https.request(
    { host: UP_HOST, port: UP_PORT, method: req.method, path: targetPath, headers, agent },
    (up) => {
      const raw = []
...
function proxyUpgrade(req, socket, head, log) {
  ...
  headers.host = `${UP_HOST}:${UP_PORT}`
  const upstream = https.connect({ host: UP_HOST, port: UP_PORT, rejectUnauthorized: false }, () => {
  ...
  register({
    ...
    handler: guarded((req, res) => {
      ...
      live.then(probe => writeJson(res, 200, {
        ...
        discoveryConfigured: envConfig.sshTarget !== null && envConfig.subnet !== null,
